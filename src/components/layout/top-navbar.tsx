...
export function TopNavbar() {
  ...
  const { isAuthenticated, user, logout, syncStatus } = useAuthStore();
        ...
        <div className="flex items-center justify-end gap-3 md:gap-6 flex-1">
          ...
          {isAuthenticated ? (
            <Link
              href="/profile"
              className={cn(
                "relative flex items-center gap-3 p-1 pr-3 rounded-full bg-white/5 border border-white/5 hover:border-pf-primary/20 transition-all group",
